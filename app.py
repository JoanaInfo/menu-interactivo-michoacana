...
# Asegúrate de que el archivo sales_data.csv esté en una carpeta llamada 'data/'
try:
    # Intenta leer el archivo
    df = pd.read_csv('data/sales_data.csv')
    df['product_id'] = df['product_id'].astype('category')
except FileNotFoundError:
    # Si falla, intenta leerlo en la carpeta raíz (donde pusiste el archivo en el prompt)
    try:
        df = pd.read_csv('sales_data.csv')
        df['product_id'] = df['product_id'].astype('category')
    except FileNotFoundError:
        print("El archivo sales_data.csv no se encontró. Asegúrate de que esté en la carpeta 'data/' o en la raíz del proyecto.")
        exit()
...
def get_weather_data(api_key, city):
    # La API Key ya estaba incluida en tu código original para prueba.
    ...
    try:
        ...
        if data.get('cod') == 200:
            ...
            if 'cloud' in weather_desc or 'mist' in weather_desc or 'fog' in weather_desc:
                ...
                return 'soleado' # Default a soleado si no se reconoce
        else:
            print(f"Error en la API del clima: {data.get('message', 'Mensaje de error desconocido')}")
    except requests.exceptions.RequestException as e:
        print(f"Error de conexión a la API del clima: {e}. Usando clima por defecto.")
...
PRODUCTS_DB = {
    ...
    # 'Fresas con Crema': Repetido, pero mantenido para la base de datos
    ...
    'Paleta de Cajeta': {'name': 'Paleta de Cajeta', 'price': '$25', 'image': 'cajeta_pop.png', 'justification': 'Una paleta dulce y cremosa de sabor tradicional.', 'category': 'Paleta de Leche'},
    'Paleta de Piña': {'name': 'Paleta de Piña', 'price': '$20', 'image': 'pinapple_pop.png', 'justification': 'Una paleta tropical, dulce y refrescante.', 'category': 'Paleta de Agua'}
}
...
@app.route('/test')
def test():
    # Mantener la ruta de prueba si la necesitas
    ...
    try:
        ...
        if chosen_product_type.lower() not in predicted_product_category.lower():
            ...
            if coherent_products:
                # Si hay productos coherentes, elegimos uno al azar
...
if __name__ == '__main__':
    # Usar host='0.0.0.0' asegura que el servidor sea accesible si se ejecuta en contenedores o VMs.
